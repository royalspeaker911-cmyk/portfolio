...
// ============================================================
// 🔧 INTERACTIVE TECH GRID — Lift + Glow + Neighbors
// Inspired by Framer InteractiveGrid component
// ============================================================
(function initTechGrid() {
  const grid = document.getElementById('techGrid');
  if (!grid) return;

  const COLS = window.innerWidth < 480 ? 3 : window.innerWidth < 768 ? 4 : 7;
  const cards = Array.from(grid.querySelectorAll('.tg-card'));
  const count = cards.length;
  let hovered = null;
  let leaveTimer = null;

  function getNeighbors(idx, cols) {
    const neighbors = [];
    if (idx % cols !== 0) neighbors.push(idx - 1);           // left
    if (idx % cols !== cols - 1) neighbors.push(idx + 1);    // right
    if (idx - cols >= 0) neighbors.push(idx - cols);          // top
    if (idx + cols < count) neighbors.push(idx + cols);       // bottom
    return neighbors;
  }

  function applyState(idx) {
    const cols = window.innerWidth < 480 ? 3 : window.innerWidth < 768 ? 4 : 7;
    cards.forEach(c => c.classList.remove('tg-big', 'tg-small'));
    if (idx === null) return;
    cards[idx].classList.add('tg-big');
    getNeighbors(idx, cols).forEach(n => cards[n].classList.add('tg-small'));
  }
  ...
  console.log('%c🔧 Tech Grid interactive — ready!', 'color: #DC143C; font-weight: bold;');
})();
